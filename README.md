...
网页支持按板块、子板块、年份、期刊、分区、IF、文章类型、评级、标题词、项目筛选，
可打分（○/⭐/🚩）、写笔记、导出 nbib 给 EndNote。**IF 和文章类型是收起式多选**：
IF 的区间互不重叠，勾多个取并集（比如同时勾「< 5」和「≥ 20」看两头）；文章类型有
60 多种，摊平成一排勾选框会把筛选栏撑爆，所以收进下拉里。
...
或输入 PMID（多个用空格/逗号分隔，一次最多 50 篇），效果与命令行 `add` 完全一致——
