...
| **③ 收藏库** | 把看中的文献存下来，可筛选、评级、写笔记、挂全文 PDF、按课题打标签；也能把 EndNote / Zotero 的旧库整个导进来 | `add` / `import-refs` / `library` / `serve` / `pdf` |
...
**报告页可以直接入库**：每篇前面有勾选框，勾完点「加入收藏库」即可，无需回终端敲
PMID。板块与子板块沿用你眼前这份报告的归类。这个按钮要 `cli.py serve` 开着（见下节）；
没开时会把等价的 `add` 命令复制到剪贴板，功能不至于全丢。
...
**已经有 EndNote / Zotero 文献库的**，可以整个导进来：

```bash
python3 cli.py import-refs --config <配置> --file ~/Downloads/My\ Library.ris
python3 cli.py import-refs --config <配置> --file refs.ris --dry-run   # 先看会收哪些
```

收藏库页面的「添加文献」弹窗里也能直接选文件，效果一样。支持 **RIS(.ris)**、
**MEDLINE/nbib(.nbib)**、**BibTeX(.bib)**、**EndNote XML(.xml)**、**CSL JSON(.json)**
（`.enw`、`.txt` 按 RIS 试）——EndNote 与 Zotero 的默认导出选项都在里头：

- Zotero：右键条目 → 导出条目 → 格式选 RIS（或 BibTeX / CSL JSON）
- EndNote：File → Export → Output style 选 RefMan (RIS) Export，或 File → Export XML

导入时**只从文件里取标识符**，书目数据仍以 PubMed 为准——导出文件里的刊名缩写、
预印本年份、缺失的 DOI 千奇百怪，直接入库会让同一篇文章因来源不同而长得不一样。
每条记录按三级路径落到 PMID：① 文件里写了 PMID（PubMed 导出、Zotero 的 Extra、
EndNote 的 accession number）→ ② 按 DOI 查 → ③ 按完整标题查。**标题在 PubMed 撞名时
宁可报错也不猜**，这类记录会原样列出来让你自己处理。书籍、网页、会议摘要本来就不在
PubMed 里，认不出属正常。

一次最多 500 条，没写 PMID 的每条都要回 PubMed 查一次，几百条要跑几分钟。
...
标题词、摘要词、项目、有无全文、入库时间筛选，可打分（○/⭐/🚩）、写笔记、挂全文 PDF、
导出 nbib 给 EndNote。

**「入库时间」筛的是你什么时候收的**（近 7 天 / 近 30 天 / 最近一次入库），与「年份、
月份」筛的发表日期是两回事——刚跑完一期周报想回看这次收了哪些，要的是前者。一批文献的
入库日期常摊在连续几天里，所以「最近一次入库」只覆盖最后那天，想看整批用「近 7 天」。
...
**入库时一律顺手试抓一次 [OA 全文](#全文-pdf)**——网页上的「添加文献」、命令行的
`add` 与 `import-refs` 都一样。能免费拿到的直接挂上，省得事后再点一次「抓取 OA 全文」；
代价是每篇多等几秒。**一篇都没抓到时也会明说**（「试抓了 N 篇，一篇也没拿到」），
不然那行提示直接不出现，看着像这功能压根没跑。抓不到是常态（订阅刊），文献本身照样
收进来了：
**全文是附加品，抓取失败绝不会让入库变成一次失败**。只试这次**新增**的那几篇——已在
库的要么早有 PDF，要么之前就试过没拿到，重试多半还是白等。

想跳过（比如一次导入几百篇、只想先把书目收进来）加 `--no-fetch`，事后再用
`pdf --fetch` 整批补：

```bash
python3 cli.py add --config <配置> --pmid 42482656 --no-fetch
```
...
在 `.env` 配 `UNPAYWALL_EMAIL`，不配就只走前一条）。**新收藏的文献入库时会自动走一遍
这个流程**，所以日常多半不用手动跑；`--fetch` 主要用来给早先收的、或当时用 `--no-fetch`
跳过的文献补全文。**订阅刊（Nature / NEJM /
Lancet 等）多半两条都抓不到，这是预期结果**，那些仍靠自己下载后拖进页面。抓下来的
一律校验 `%PDF` 文件头——出版商的拦截页是 200 + HTML，不验就会存下一堆「请登录」。

同一篇文章常有多份 OA 副本，会**把仓储副本排在出版商官网前面**逐个试：出版商站点
普遍拦非浏览器客户端，而 PMC / 机构仓储的那份直接就能下。抓取并发 2 路——串行时
单篇失败要走完重试退避，几百篇能拖上一个多小时；再快则会被 Unpaywall 限流。
...
| `add` | 按 PMID 存入收藏库 |
| `import-refs` | 导入 EndNote / Zotero 导出的文件（`--file`，`--dry-run` 先看） |
